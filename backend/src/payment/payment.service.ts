...
export class PaymentService {
  ...
  async create(createPaymentDto: CreatePaymentDto) {
    ...
    if (createPaymentDto.data) {
      ...
      const totalPrice = labData
        .reduce((sum, item) => sum + (item.service_price || 0), 0)
        .toFixed(2);
  ...
  async findAllPendingPayment() {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      let services = await this.dataSource.manager
        ...
        .where('p.id IN (:ids)', { ids: data[i].all_paymentIDs.split(', ') })
        .getMany();
      ...
      Object.assign(data[i], { services_availed: services_arr });
    }
  ...
  async fullyPaid() {
    ...
    // console.log(data);
    for (let i = 0; i < data.length; i++) {
      let paymentIDs = JSON.parse(data[i].payedId);
  ...
  async getAnalyticsIncome(selectedDate?: string) {
    ...
    if (selectedDate) {
      ...
      // default: this month
      dailyQuery = dailyQuery.andWhere(
        'MONTH(pay.created_at) = MONTH(CURDATE())',
      );
    }
  ...
  async getDailyIncome(date: string) {
    ...
    if (date) {
      ...
      // default: this month
      dailyQuery = dailyQuery.andWhere('DATE(pay.created_at) = DATE(NOW())');
    }
  ...
  async getMonthlyIncome(month: string, year: string) {
    let query = this.dataSource.manager
      .createQueryBuilder(Payment, 'pay')
      .where('pay.status = 1')
      .andWhere('MONTHNAME(pay.created_at) = :month', { month })
      .andWhere('YEAR(pay.created_at) = :yr', { yr: year })
      .getMany();

    return query;
  }

  async getYearlyIncome(year: string) {
    let query = this.dataSource.manager
      .createQueryBuilder(Payment, 'pay')
      .where('pay.status = 1')
      .andWhere('YEAR(pay.created_at) = :yr', { yr: year })
