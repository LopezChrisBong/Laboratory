...
export class AppointmentService {
  constructor(
    ...
    @InjectRepository(PatientMedtech)
    private readonly patientMedtechRepository: Repository<PatientMedtech>,
    @InjectRepository(ServiceAppointment)
    private readonly serviceAppointmentRepository: Repository<ServiceAppointment>,
    @InjectRepository(ServiceLabResult)
    private readonly serviceLabResultRepository: Repository<ServiceLabResult>,
  ...
  async bookAppointment(createAppointmentDto: CreateAppointmentDto) {
    ...
    try {
      ...
      const isExist = await this.appointmentRepository.findOne({
        where: { patientID: appointment.patientID, status: In([0, 1]) },
      });
      ...
      if (!isExist) {
        const data = queryRunner.manager.create(Appointment, {
          ...
          status: appointment.status
            ? appointment.status
            : appointment.clinic != 'Others'
            ? 1
            : 0,
          doctorID: appointment.doctorID,
        });
        let save = await queryRunner.manager.save(data);
        let patient = await queryRunner.manager
          .createQueryBuilder(Patient, 'pt')
          .where('pt.id = :id', { id: appointment.patientID })
          .getOne();
        let doctor = await queryRunner.manager
  ...
  async bookDoctorPatientAppointment(
  ...
  async addPatientDoctor(createPatientDoctorDto: CreatePatientDoctorDto) {
    ...
    try {
      ...
      if (infoID.appointmentID) {
        ...
        const isExist = await this.patientDoctorRepository.findOneBy({
  ...
  async addPatientMedTech(createPatientMedtechDto: CreatePatientMedtechDto) {
    ...
    try {
      ...
      if (infoID.appointmentID) {
        const isExist = await this.patientMedtechRepository.findOneBy({
          appointmentID: infoID.appointmentID,
          patientID: infoID.patientID,
        });
  ...
  async getAssignedBookedAppointmentDoctor(id: number, patientID: number) {
    const data = await this.serviceAppointmentRepository
      .createQueryBuilder('sa')
      .select(['sa.*', 'us.liscence_no AS liscence_no'])
      .leftJoin(Patient, 'pt', 'pt.id = sa.patientID')
  ...
  async getAllScheduleData() {
    let data = await this.appointmentRepository
      .createQueryBuilder('ap')
      .select(['ap.date AS date', 'ap.time AS time'])
      .getRawMany();
  ...
  async getAllAppointmentDashboard() {
    ...
    let totalPatient30Days = await this.appointmentRepository
      ...
      .where('ap.medtechID IS NULL')
      .andWhere(
        'ap.date BETWEEN DATE_SUB(CURDATE(), INTERVAL 30 DAY) AND CURDATE()',
      )
      .andWhere('ap.status != 0')
      .getRawMany();
    ...
    return {
      ...
      totalPatient30Days: totalPatient30Days,
  ...
  async updateServiceResult(
    id: number,
    updateServiceResultsDto: UpdateServiceResultsDto,
  ) {
  ...
  async confirmAppointment(
    ...
    try {
      ...
      if (updated.affected == 1) {
        let patient = await this.dataSource.manager
          .createQueryBuilder(Patient, 'pt')
