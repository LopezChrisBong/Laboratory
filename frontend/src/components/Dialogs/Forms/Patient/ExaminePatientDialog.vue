                ...
                >
                  ...
                  <template v-slot:[`item.action`]="{ item }">
                    <v-btn
                      ...
                      color="green"
                      block
                      ><v-icon small> mdi-eye</v-icon> Examine</v-btn
                    >
                    <v-btn
                      ...
                      block
                    >
                      <v-icon small> mdi-pencil</v-icon> Examine</v-btn
                    >
                    <v-btn
          ...
          <v-simple-table dense class="text-caption">
            ...
            <tbody>
              <tr v-for="(item, index) in service" :key="index">
                ...
                <td>{{ item.unit || "-" }} 33</td>
                <td>{{ item.remarks || "-" }} 33</td>
              </tr>
              <tr v-for="(item, index) in newPackageData" :key="index">
...
export default {
  ...
  methods: {
    ...
    submitResult() {
      ...
      // let data = {
      //   service_result: JSON.stringify(this.input),
      //   package_result: JSON.stringify(this.input1),
      // };

      // this.axiosCall(
      //   "/appointment/updateServiceResult/" + this.updateID,
      //   "PATCH",
      //   data
      // ).then((res) => {
      //   console.log(res);
    ...
    edit(item) {
      ...
      let packageData = JSON.parse(item.service_package);
      this.service = JSON.parse(item.service);
      console.log(this.service);

      let promises = packageData.map((pkg) =>
        this.axiosCall("/services/getSpecificPackages/" + pkg.id, "GET")
      );

      Promise.all(promises).then((responses) => {
        this.newPackageData = []; // reset the array

        responses.forEach((res) => {
          let assignMods = res.data.assign_mods;

          // Parse if needed
          try {
            assignMods =
              typeof assignMods === "string"
                ? JSON.parse(assignMods)
                : assignMods;
          } catch (err) {
            console.error("Invalid JSON in assign_mods:", assignMods);
            assignMods = null;
          }

          // ✅ Push elements directly (flatten here)
          if (Array.isArray(assignMods)) {
            this.newPackageData.push(...assignMods); // flattening here
          } else if (assignMods) {
            this.newPackageData.push(assignMods);
          }
        });

        console.log("PAKKKG", this.newPackageData); // will be flat
